#!/usr/bin/env python3
"""A simple Flask application with localization, timezone support, and
...
content accordingly, including the current time formatted in the user's
locale.
...
def before_request() -> None:
    """Set user data and formatted time in the global context before request.

    This function runs before every request to make user information
    and the current time accessible in templates and routes by storing
    them in `g.user` and `g.time`, respectively.
    """
    user = get_user()
    g.user = user
